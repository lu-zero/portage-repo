...
if [[ ! -x "$REGEN" ]]; then
    echo "Building regen_only..." >&2
    cargo build --release --example regen_only --manifest-path "$SCRIPT_DIR/Cargo.toml"
fi
...
for J in "${JOBS[@]}"; do
    OUT=$(mktemp -d)
    { time "$REGEN" "$REPO" -o "$OUT" -j "$J" >/dev/null 2>&1; } 2>/tmp/bench_time &
    BGPID=$!
    RSS=$(peak_rss "$BGPID")
    wait "$BGPID"
